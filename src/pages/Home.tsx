    ...
    <main className="site-page pt-16">
      <section className="brand-grid border-b border-[#2D3142]/15">
        <div className="page-shell grid min-h-[650px] items-center gap-14 py-20 lg:grid-cols-[1.18fr_.82fr] lg:py-24">
          ...
          <div className="mx-auto w-full max-w-[440px] lg:justify-self-end">
            <div className="grid grid-cols-[1fr_auto] items-start gap-x-6 border-b border-[#2D3142]/25 pb-3 mono text-[10px] uppercase tracking-[.06em] text-[#4F5D75]"><span>System_Status<br />Node_01</span><span className="text-[#EF8354]">Active</span></div>
            ...
            <div className="mt-12 flex h-28 items-end gap-2.5 border-b border-[#2D3142]/8 pb-px">{bars.map((height, index) => <span key={`${height}-${index}`} className="flex-1 bg-[#EF8354]/20" style={{ height: `${height}%`, opacity: .48 + index * .055 }} />)}</div>
          </div>
        </div>
      </section>
      ...
      <section className="paper border-b border-[#2D3142]/15">
        <div className="page-shell py-20 lg:py-24">
          <div className="mb-14"><p className="mono text-[9px] uppercase tracking-[.12em] text-[#EF8354]">Flagship Product</p><h2 className="mt-3 text-[clamp(2rem,3.4vw,3.4rem)] font-semibold tracking-[-.045em]">Cognisense Cloud</h2><p className="mt-1 text-[13px] text-[#4F5D75]">Give every camera intelligence.</p></div>
          ...
          <div className="grid gap-12 lg:grid-cols-[1.08fr_.92fr] lg:items-center">
            ...
            <div className="lg:pl-4">
              ...
              <div className="mt-8 space-y-6">{cognisenseFeatures.map((feature) => <div key={feature.title} className="grid grid-cols-[22px_1fr] gap-3"><feature.icon className="mt-0.5 h-4 w-4 text-[#EF8354]" strokeWidth={1.7} /><div><p className="text-[12px] font-medium">{feature.title}</p><p className="mt-1 text-[11px] leading-5 text-[#4F5D75]">{feature.copy}</p></div></div>)}</div>
      ...
      <section className="brand-grid border-b border-[#2D3142]/15">
        <div className="page-shell py-20 lg:py-24">
          ...
          <div className="mt-12 grid gap-px border border-[#2D3142]/15 bg-[#2D3142]/15 md:grid-cols-2">{capabilityCards.map((item) => <article key={item.id} className="bg-white p-7 sm:p-9"><div className="flex items-center justify-between"><span className="mono text-[9px] uppercase tracking-[.08em] text-[#4F5D75]">Capability_{item.id}</span><item.icon className="h-4 w-4 text-[#EF8354]" strokeWidth={1.6} /></div><h3 className="mt-6 text-lg font-medium tracking-[-.02em]">{item.title}</h3><p className="mt-3 max-w-[520px] text-[12px] leading-6 text-[#4F5D75]">{item.copy}</p><div className="mt-8 flex flex-wrap gap-2">{item.tags.map((tag) => <span className="tech-tag" key={tag}>{tag}</span>)}</div></article>)}</div>
          <div className="mt-8 grid gap-6 border-t border-[#2D3142]/20 pt-7 md:grid-cols-[1fr_auto] md:items-start"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 text-[#EF8354]" /><p className="max-w-2xl text-[11px] leading-5 text-[#4F5D75]">Engineering discipline across model, platform, interface, deployment and operations—not isolated prototypes.</p></div><div className="mono text-[9px] uppercase leading-5 tracking-[.08em] text-[#4F5D75]">Latest_Commit<br /><span className="text-[#2D3142]">Operational intelligence system</span></div></div>
        </div>
      </section>
